...
DECLARE_ALGORITHM(GPPDMAPMTRecAlg);

GPPDMAPMTRecAlg::GPPDMAPMTRecAlg(const std::string& name)
    : AlgBase(name)
{
    m_count = 0;
}

GPPDMAPMTRecAlg::~GPPDMAPMTRecAlg()
{
}

bool
GPPDMAPMTRecAlg::initialize()
{
...
void GPPDMAPMTRecAlg::reconstruct(int time){
	int X=0;
	int Y=0;
	int effect=0;
...
int GPPDMAPMTRecAlg::calculation(int &X,int &Y,int &Effect)
{
...
int GPPDMAPMTRecAlg::scan(int *Pos,int l,int &ThePosition,int &Real){
...
int GPPDMAPMTRecAlg::sort(int *src, int len){
